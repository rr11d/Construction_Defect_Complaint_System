...
async function generateDiagnosis(detectionText, userMessage = '', imagePath = null) {
  ...
  const systemPrompt = `당신은 건설 하자 진단 전문가다.
# 입력
1. YOLO 자동탐지 텍스트 (매우 약한 보조 신호)
2. 원본 사진 (주요 판단 근거)

# 절대 규칙
YOLO가 "탐지 0건"이라고 보고해도 신뢰하지 말 것. 사진을 직접 보고 판단하라.

# 진단 기준
사진에서 다음 중 하나라도 보이면 적극적으로 진단하라.
...
# severityScore 규칙
- 사진에 어떤 형태든 이상(얼룩/변색/누런 자국 포함)이 보이면 절대 1 이하 금지.
- 명확한 누수/균열/곰팡이 → 5-8
- 의심 단서 → 3-4
- 100% 깨끗한 새 벽/천장일 때만 → 1-2

# 출력 형식
설명·마크다운·코드블록 없이 아래 JSON 객체 하나만 응답하라.
{
